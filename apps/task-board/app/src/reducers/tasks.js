import {
  LOAD_FULL_STATE, RELOAD_FULL_STATE, ADD_TASK, REMOVE_TASK, UPDATE_TASK, ACCEPT_TASK, FINISH_TASK, RELOAD_TASK,
  MOVE_TASK, REWARD_TASK, ERROR_TASK, CLEAR_ERROR, LOAD_ACCOUNT
} from '../actions/tasks'
...
const reduceFullState = (state, contractState) => {
  if(contractState.tasks) contractState.tasks = contractState.tasks.map(task => new Task(task._id, task._workField, task._heading, task._description, task._project, task._tags, task._difficulty, task._column, task._tokens, task._assignee, task._issuer, TASK_TYPES.BASE, task._date))
...
const acceptTask = (state, payload) => {
  var newTasks = state.tasks.map(item => {
    ...
    var task = new Task(item._id, item._workField,
      item._heading, item._description, item._project, item._tags,
      item._difficulty,
...
const removeTask = (state, payload) => {
  const tasks = state.tasks.map(item => {
    ...
    var task = new Task(item._id, item._workField,
      item._heading, item._description, item._project, item._tags,
      item._difficulty, item._column, item._tokens, item._assignee,
      item._issuer, TASK_TYPES.DELETED)
    return task
  })
  return { ...state, tasks }
}
...
const updateTask = (state, payload) => {
  const tasks = state.tasks.map(item => {
    ...
    const task = new Task(payload.task._id, payload.task._workField, payload.task._heading, payload.task._description, payload.task._project,
      payload.task._tags, payload.task._difficulty, payload.task._column, payload.task._tokens, payload.task._assignee, payload.task._issuer)
    return task
  })
  return { ...state, tasks }
}
...
const finishTask = (state, payload) => {
  var newTasks = state.tasks.map(item => {
    ...
    var task = new Task(item._id, item._workField,
      ...
      item._difficulty,
      3, //updating column
      item._tokens, state.currentUser,
      item._issuer)
    return task
  })
  return { ...state, tasks: [...newTasks] }
...
const reloadTask = (state, payload) => {
  if (payload.task.type != TASK_TYPES.NEW) {
    ...
    const index = state.tasks.indexOf(state.tasks.filter(task => task.id == payload.task.id)[0])
...
const errorTask = (state, payload) => {
  ...
  if (!check) {
    return { ...state, errors: [{ id: payload.task.id, error: payload.error }, ...state.errors] }
  }
  return { ...state, errors }
}
const clearError = (state, payload) => {
  const index = state.errors.indexOf(state.errors.filter(error => error.id == payload.task.id)[0])
  return { ...state, errors: [...state.errors.slice(0, index), ...state.errors.slice(index + 1)] }
}